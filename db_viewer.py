    ...
    support_content = """
      <div class="help-layout">
        ...
        <div class="help-main">
          ...
          <section class="support-block" id="support-project"><div class="section-heading"><span class="section-kicker">07 · Развитие проекта</span><h2>Поддержать Podslushka DB</h2><p>Поддержка помогает оплачивать инфраструктуру и развивать новые функции.</p></div><div class="wallets"><div><b>TON</b><code>UQBkuXi2eXAG3py9XDyC7V0AMO8iuqKclDJ_emGaL2vIGFOO</code></div><div><b>ERC20 · BEP20</b><code>0x79C6fa74C4634F244ac39dB52110359fdF57ACC6</code></div><div><b>SOLANA</b><code>8Rhe4Gvtfo4CywLUAXEr76hXs2yGGaTrBG7VEUxZTY9p</code></div><div><b>TRC20</b><code>TRErYRVTKRRaPdw5vQ6yHSGu3iKCqT5z1h</code></div></div></section>
        </div>
      </div>
      <script>document.querySelectorAll('.wallets code').forEach(function(item){{item.title='Нажмите, чтобы скопировать';item.addEventListener('click',function(){{navigator.clipboard.writeText(item.textContent).then(function(){{var old=item.textContent;item.textContent='Скопировано ✓';setTimeout(function(){{item.textContent=old}},1200)}})}})}});</script>
...
.support-block{{margin-top:26px}}.wallets{{grid-template-columns:repeat(2,1fr)}}.wallets div{{padding:14px;border:1px solid var(--line);border-radius:10px;background:#1e1214dd}}.wallets b{{display:block;color:var(--accent);margin-bottom:8px;font-size:13px}}code{{display:block;overflow:auto;color:#ffe9d2;font:12px Consolas,monospace;white-space:nowrap}}
...
body.theme-transparent .layout{{background:linear-gradient(135deg,#0b1624aa,#111d2aaa)!important}}
body.theme-transparent .sidebar,body.theme-transparent .card,body.theme-transparent .toolbar,body.theme-transparent .insight-card,body.theme-transparent .table-wrap,body.theme-transparent .action-shell{{background:var(--theme-panel)!important;backdrop-filter:blur(18px)}}
body.theme-transparent .card,body.theme-transparent .toolbar,body.theme-transparent .insight-card,body.theme-transparent .table-wrap{{box-shadow:0 18px 40px #0005}}
