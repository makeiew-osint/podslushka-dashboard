...
body:before,body:after{{content:"";position:fixed;pointer-events:none;border:1px solid #7d9dff55;filter:drop-shadow(0 0 18px #477dff55);z-index:0}}
...
.shell{{position:relative;width:min(920px,100%);display:grid;grid-template-columns:1fr 1fr;overflow:hidden;border:1px solid #2a4265;
border-radius:24px;background:linear-gradient(145deg,#1c2045dd,#10152ddd);box-shadow:28px 34px 0 #050611aa,0 24px 80px #000b,0 0 0 1px #8d7cff44;backdrop-filter:blur(18px);transform:rotateX(2deg) rotateY(-2deg);transform-style:preserve-3d;animation:loginEnter .8s cubic-bezier(.2,.8,.2,1) both}}
...
@keyframes loginEnter{{from{{opacity:0;transform:rotateX(9deg) rotateY(-9deg) translateY(28px) scale(.96)}}to{{opacity:1;transform:rotateX(2deg) rotateY(-2deg) translateY(0) scale(1)}}}}@keyframes loginSweep{{0%,55%{{transform:translateX(-120%)}}78%,100%{{transform:translateX(120%)}}}}@keyframes loginOrb{{50%{{transform:translate3d(-20px,-18px,20px) scale(1.12)}}}}@keyframes loginOrbitLeft{{to{{transform:rotateX(66deg) rotateZ(336deg)}}}}@keyframes loginCube{{to{{transform:rotateX(360deg) rotateY(360deg) rotateZ(180deg)}}}}.auth:after{{content:"";position:absolute;width:82px;height:82px;right:28px;bottom:24px;border:1px solid #7ea9ff66;border-radius:22px;transform:rotate(35deg);animation:loginCube 14s linear infinite;pointer-events:none}}.auth{{position:relative;overflow:hidden}}.auth input,.auth .submit,.oauth-button,.telegram-button{{transform-style:preserve-3d}}.auth input:hover{{transform:translateY(-2px) translateZ(5px)}}.auth h2{{text-shadow:0 4px 18px #7b61ff66}}@media(max-width:700px){{.shell{{grid-template-columns:1fr;max-width:460px}}.intro{{padding:30px}}.intro h1{{margin:28px 0 12px;font-size:29px}}.features{{display:none}}.auth{{padding:30px}}}}
