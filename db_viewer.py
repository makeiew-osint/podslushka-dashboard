...
def user_detail_page(current_user: str, user_id: int,
    ...
    posts_html = "".join(
        ...
        f"{' disabled title=\"' + esc(AI_MAINTENANCE_TEXT) + '\"' if AI_MAINTENANCE_MODE else ''}>"
        f"{'ИИ: техработы' if AI_MAINTENANCE_MODE else ('ИИ-анализ ✓' if row['ai_analysis'] else 'ИИ-анализ')}</button></td></tr>" for row in posts
...
def page(current_user: str = "", section: str = "overview", history_post_id: str = "", selected_bot_id: str = "", impersonated_by: str = "") -> str:
    role = dashboard_role(current_user)
    owner = role == "owner"
    ai_maintenance_attr = " disabled" if AI_MAINTENANCE_MODE else ""
    ai_mode_label = "ИИ: техработы" if AI_MAINTENANCE_MODE else "С ИИ"
    ai_mode_help = "Временно недоступно" if AI_MAINTENANCE_MODE else "Краткое резюме и анализ"
    ...
    post_rows = "".join(
        ...
        f"<td><button type=\"button\" class=\"ai-analysis-button\" data-post-id=\"{esc(row['id'])}\""
        f"{' disabled title=\"' + esc(AI_MAINTENANCE_TEXT) + '\"' if AI_MAINTENANCE_MODE else ''}>"
        f"{'ИИ: техработы' if AI_MAINTENANCE_MODE else ('ИИ-анализ ✓' if row['ai_analysis'] else 'ИИ-анализ')}</button></td>"
        "</tr>"
        for row in posts
    )
...
<title>Podslushka DB</title><link rel="icon" type="image/svg+xml" href="/assets/podslushka-favicon.svg"><link rel="apple-touch-icon" href="/assets/podslushka-avatar.svg"><meta name="theme-color" content="#0b1220"><style>
...
.bot-switcher{{display:inline-flex;max-width:100%;align-items:center;gap:8px;flex-wrap:wrap;margin:0 0 18px;padding:9px 11px;border:1px solid #254873;border-radius:14px;background:#0b192d;box-shadow:0 10px 24px #02071355;animation:cardIn .45s both}}.bot-switcher span{{color:#85a4c9;font-size:10px;text-transform:uppercase;letter-spacing:.7px;margin-right:3px}}.bot-switcher a{{padding:7px 11px;border:1px solid #2d5787;border-radius:9px;background:#102542;color:#bcd0f1;box-shadow:none}}.bot-switcher a.active{{background:linear-gradient(135deg,#2679e9,#6d5cf0);color:#fff;border-color:#80a9ff;box-shadow:0 5px 18px #357df455}}@keyframes modelFloat{{0%,100%{{transform:rotate(35deg) translateY(0) translateZ(0)}}50%{{transform:rotate(55deg) translateY(12px) translateZ(18px)}}}}@keyframes orbFloat{{0%,100%{{transform:translate3d(0,0,0) scale(1)}}50%{{transform:translate3d(-10px,-12px,18px) scale(1.12)}}}}@keyframes cubeFloat{{0%,100%{{transform:rotateX(-18deg) rotateY(0deg) translateY(0)}}50%{{transform:rotateX(18deg) rotateY(180deg) translateY(-9px)}}}}@keyframes ringSpin{{to{{transform:rotateX(68deg) rotateZ(360deg)}}}}
...
.osint-search-form fieldset{{display:flex;align-items:center;gap:10px;min-width:0;min-height:58px;margin:0;border:1px solid #173b68;padding:10px 12px;border-radius:16px;background:#071426cc;overflow:hidden}}
...
.osint-search-form fieldset label{{display:flex;align-items:center;gap:8px;padding:11px 12px;border:1px solid #14365d;border-radius:12px;color:var(--text);font-size:12px;font-weight:700;text-transform:none;letter-spacing:0;cursor:pointer;transition:background .18s,border-color .18s,transform .18s}}
...
.osint-search-form fieldset label span{{display:grid;gap:3px;min-width:0;overflow:hidden}}
...
.osint-search-form .osint-mode{{display:grid!important;grid-template-columns:repeat(3,minmax(0,1fr));grid-column:1;grid-row:2;align-content:stretch;min-width:0}}
.osint-search-form .osint-mode{{min-width:0}}
.osint-search-form .osint-mode label{{flex:1 1 0;min-width:0;min-height:58px}}
...
.osint-search-form button.osint-launch{{display:flex!important;align-items:center;justify-content:center;position:relative!important;z-index:2!important;background:linear-gradient(105deg,#218bf6,#5268f5 58%,#7545e8)!important;color:#fff!important;border-color:#4c86ff!important}}
...
.osint-search-form::before{{content:"";position:absolute;inset:-1px;border-radius:24px;pointer-events:none;background:linear-gradient(115deg,#3ca4ff55,transparent 35%,#7b4dff44);opacity:.55;z-index:-1}}
.osint-search-form{{position:relative}}
.osint-tool-card{{display:flex!important;align-items:center;justify-content:space-between;flex:1 1 0;min-width:0;min-height:64px;padding:10px 14px!important;background:linear-gradient(145deg,#0d2340,#0a172c)!important}}
.osint-tool-card input{{accent-color:#2f9bff}}
.osint-tool-card span{{display:grid;gap:4px}}
.osint-tool-card b{{font-size:13px}}
.osint-tool-card small{{color:#86a8d2;font-size:11px;font-weight:500}}
.osint-mode-card{{display:flex!important;align-items:center;justify-content:flex-start;gap:10px;min-width:0;min-height:62px;padding:10px 13px!important;overflow:hidden}}
.osint-mode-card input{{position:relative;z-index:1;flex:0 0 auto;width:16px!important;min-width:16px!important;margin:0!important}}
.osint-mode-card span{{display:block!important;min-width:0!important;overflow:hidden!important;color:var(--text)!important;text-overflow:ellipsis;white-space:nowrap}}
.osint-mode-card span b,.osint-mode-card span small{{display:block!important;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}}
.osint-mode-card b{{font-size:13px}}
.osint-mode-card small{{display:block;color:#86a8d2;font-size:11px;font-weight:500;margin-top:3px}}
.osint-results{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}}
.osint-results-shell{{margin-top:18px;padding:22px;border:1px solid #1e4d84;border-radius:22px;background:linear-gradient(145deg,#07162bdf,#0a1123ed);box-shadow:0 18px 50px #01071366}}
.osint-results-shell h3{{margin:0 0 6px;font-size:17px}}
.osint-results-shell>p{{margin:0 0 16px;color:#86a8d2;font-size:13px}}
.osint-live-grid{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}}
.osint-live-card{{position:relative;overflow:hidden;padding:15px;border:1px solid #173c68;border-radius:15px;background:linear-gradient(145deg,#0b203b,#081426);transition:transform .2s,border-color .2s,box-shadow .2s}}
.osint-live-card:hover{{transform:translateY(-2px);border-color:#327ec8;box-shadow:0 12px 28px #02071366}}
.osint-live-card header{{display:flex;justify-content:space-between;gap:8px;margin-bottom:10px}}
.osint-live-card .dot{{display:inline-block;width:7px;height:7px;margin-right:6px;border-radius:50%;background:#29d391;box-shadow:0 0 12px #29d391}}
.osint-live-card.pending .dot{{background:#7388a8;box-shadow:none}}
.osint-live-card small{{color:#89a9cf}}
.osint-result,.osint-summary{{padding:16px;border:1px solid #1a4776;border-radius:16px;background:linear-gradient(145deg,#0b1d35,#091426);box-shadow:0 10px 30px #02071344}}
...
.osint-result a{{display:flex;align-items:center;gap:9px;padding:10px 11px;border:1px solid #15375d;border-radius:11px;color:#9dcaff;background:#071426aa;text-decoration:none;overflow-wrap:anywhere;transition:background .2s,border-color .2s,transform .2s}}
.osint-result a span{{display:block;color:#d7eaff;font-weight:700}}
.osint-result a small{{display:block;margin-top:3px;color:#7192b7;font-size:11px;overflow-wrap:anywhere}}
.osint-result a::before{{content:"↗";display:grid;place-items:center;width:23px;height:23px;flex:0 0 23px;border-radius:7px;color:#63b4ff;background:#164d80;font-size:12px}}
.osint-result a:hover{{border-color:#3a96e9;background:#0d2747;transform:translateX(2px)}}
.osint-result .status{{padding:4px 8px;border-radius:999px;background:#123a2f;color:#7ce4b4;font-size:11px}}
.osint-result .all-info-error{{margin:10px 0 0}}
...
.osint-header{{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;margin-bottom:20px}}
.osint-title{{display:flex;align-items:flex-start;gap:14px}}
.osint-title-icon{{display:grid;place-items:center;width:48px;height:48px;flex:0 0 48px;border:1px solid #2f78c0;border-radius:15px;background:linear-gradient(145deg,#123e77,#231d75);box-shadow:0 10px 28px #175db044;color:#80c7ff;font-size:24px}}
.osint-title h2{{margin:0 0 6px;font-size:28px;letter-spacing:-.04em}}
.osint-title h2 span{{background:linear-gradient(100deg,#51a9ff,#9a72ff);-webkit-background-clip:text;background-clip:text;color:transparent}}
.osint-title p{{max-width:720px;margin:0;color:#8199b8;font-size:13px}}
.osint-public{{display:flex;align-items:center;gap:8px;padding:10px 13px;border:1px solid #24466c;border-radius:13px;background:#0b1a2eaa;color:#b7cbe4;font-size:12px;font-weight:700;white-space:nowrap}}
.osint-public i{{width:7px;height:7px;border-radius:50%;background:#48d69d;box-shadow:0 0 11px #48d69d}}
.osint-card-heading{{display:flex;align-items:center;gap:10px;margin-bottom:12px}}
.osint-card-heading h3{{margin:0;font-size:15px}}
.osint-card-heading p{{margin:3px 0 0;color:#7e9abd;font-size:11px}}
.osint-card-heading .glyph{{display:grid;place-items:center;width:32px;height:32px;border-radius:10px;background:#123d70;color:#6eb9ff}}
.osint-results-empty{{display:flex;align-items:center;justify-content:space-between;gap:22px;min-height:118px;padding:24px;border:1px dashed #28527d;border-radius:16px;background:radial-gradient(circle at 84% 40%,#29468c33,transparent 28%),#081526aa}}
...
@media(max-width:1100px){{.osint-search-form{{grid-template-columns:1fr;grid-template-rows:auto auto auto auto}}.osint-search-form>label{{grid-column:1;grid-row:1}}.osint-search-form>fieldset:not(.osint-mode){{grid-column:1;grid-row:2}}.osint-search-form .osint-mode{{grid-column:1;grid-row:3}}.osint-search-form>.submit{{grid-column:1;grid-row:4}}.osint-results{{grid-template-columns:repeat(2,minmax(0,1fr))}}}}
...
<main class="content">{impersonation_notice}<div class="topbar"><div class="topbar-title"><button type="button" class="mobile-menu-button" id="mobile-menu-button" aria-label="Открыть меню">☰</button><div><h1>Панель управления</h1><div class="muted">Мониторинг базы данных и модерации · роль: <b>{esc(role)}</b></div></div></div><div class="topbar-actions"><select id="ai-provider" aria-label="Провайдер ИИ" {'disabled' if AI_MAINTENANCE_MODE else ''}><option value="gemini" {'selected' if AI_PROVIDER == 'gemini' else ''} {'disabled' if AI_MAINTENANCE_MODE or not GEMINI_API_KEY else ''}>ИИ: {'техработы' if AI_MAINTENANCE_MODE else ('Gemini · доступен' if GEMINI_API_KEY else 'Gemini · не настроен')}</option><option value="qwen" {'selected' if AI_PROVIDER in {'qwen', 'huggingface', 'hf'} else ''} {'disabled' if AI_MAINTENANCE_MODE or not HF_TOKEN else ''}>ИИ: {'техработы' if AI_MAINTENANCE_MODE else ('Qwen · доступен' if HF_TOKEN else 'Qwen · не настроен')}</option><option value="deepseek" {'selected' if AI_PROVIDER == 'deepseek' else ''} {'disabled' if AI_MAINTENANCE_MODE or not HF_TOKEN else ''}>ИИ: {'техработы' if AI_MAINTENANCE_MODE else ('DeepSeek · доступен' if HF_TOKEN else 'DeepSeek · не настроен')}</option><option value="glm" {'selected' if AI_PROVIDER == 'glm' else ''} {'disabled' if AI_MAINTENANCE_MODE or not HF_TOKEN else ''}>ИИ: {'техработы' if AI_MAINTENANCE_MODE else ('GLM-5.3 · доступен' if HF_TOKEN else 'GLM-5.3 · не настроен')}</option></select>{'<span class="ai-provider-note" title="' + esc(AI_MAINTENANCE_TEXT) + '">⚠ ИИ: технические работы</span>' if AI_MAINTENANCE_MODE else ''}<select id="refresh-interval" aria-label="Частота обновления"><option value="5">Обновление: 5 сек</option><option value="15">Обновление: 15 сек</option><option value="30">Обновление: 30 сек</option><option value="60">Обновление: 1 мин</option><option value="0">Обновление выключено</option></select><select id="interface-language" aria-label="Язык интерфейса"><option value="ru">Русский</option><option value="en">English</option></select><button type="button" id="compact-mode-button">Компактный режим</button><a class="button-link" href="/profile">◉ Профиль</a><a class="button-link" href="/export/users.csv">↓ CSV</a><a class="button-link danger" href="/logout">Выйти</a></div></div>
...
function applyTheme(theme) {{
  ...
  localStorage.setItem('podslushka-theme', theme);
...
function renderOsint(job) {{
  ...
  const known = Object.fromEntries((job.results || []).map(group => [group.tool, group]));
  const liveCards = (job.tools || []).map(tool => {{
    const group = known[tool];
    const ready = group && group.status !== 'running';
    const count = group ? (group.results || []).length : 0;
    const countLabel = ready ? String(count) + ' ссылок' : 'Проверяется…';
    const detail = group && group.error ? escapeHtml(group.error) : (ready ? 'Источник обработан' : 'Ищем публичные профили');
    return '<article class="osint-live-card ' + (ready ? '' : 'pending') + '"><header><b><span class="dot"></span>' + (toolLabels[tool] || escapeHtml(tool)) + '</b><small>' + countLabel + '</small></header><small>' + detail + '</small></article>';
  }}).join('');
  if (job.status === 'running') {{
    const ready = (job.results || []).filter(group => group.status !== 'running').length;
    osintStatus.textContent = `Поиск выполняется: готово ${{ready}} из ${{(job.tools || []).length}} источников…`;
  }}
  ...
  if (job.status === 'completed') osintStatus.textContent = `Проверка @${{escapeHtml(job.username)}} завершена.`;
...
async function requestAiAnalysis(button) {{
  ...
  if ({str(AI_MAINTENANCE_MODE).lower()}) {{
    showAiError({json.dumps(AI_MAINTENANCE_TEXT, ensure_ascii=False)});
    ...
    def do_POST(self):
        ...
        if path == "/api/osint-search":
            ...
            try:
                ...
                if ai and AI_MAINTENANCE_MODE:
                    raise RuntimeError(AI_MAINTENANCE_TEXT)
                ...
                self.send_response(503 if AI_MAINTENANCE_MODE and ai else 429)
                self.send_header("Content-Type", "application/json; charset=utf-8")
                self.send_header("Cache-Control", "no-store")
                self.send_header("Content-Length", str(len(body)))
                self.end_headers()
