...
.osint-search-form{{display:grid;grid-template-columns:minmax(300px,.9fr) minmax(460px,1.8fr);align-items:stretch;gap:14px;margin:20px 0;padding:16px;border:1px solid #214d83;border-radius:24px;background:linear-gradient(135deg,#071b35f5 0%,#0b1730f2 55%,#111348f5 100%);box-shadow:0 22px 55px #02071388,0 0 70px #1d64d51c inset}}
...
{('<section id="user-search"><div class="section-heading"><div><h2>⌕ Поиск <span style="color:#65a7ff">пользователя</span></h2><p class="muted">Проверьте username только по открытым веб-источникам. Найдите аккаунты, связанные с этим пользователем.</p></div><span class="status">◉ Публичные данные</span></div><form class="osint-search-form" id="osint-search-form" onsubmit="return false" novalidate><label><strong style="font-size:15px;color:var(--text);text-transform:none;letter-spacing:0">Username</strong><small class="muted" style="text-transform:none;letter-spacing:0;font-size:12px">Введите имя пользователя, никнейм или @username</small><input id="osint-username" name="username" placeholder="@username" maxlength="32" autocomplete="off" required><small class="muted">● Например: @username или username</small></label><fieldset><legend>⌘ Источники поиска</legend><label><input type="checkbox" name="tool" value="blackbird" checked><span>◉ <b>Blackbird</b><small>400+ сайтов</small></span></label><label><input type="checkbox" name="tool" value="maigret" checked><span>◉ <b>Maigret</b><small>2500+ сайтов</small></span></label><label><input type="checkbox" name="tool" value="sherlock" checked><span>◉ <b>Sherlock</b><small>400+ соцсетей</small></span></label></fieldset><fieldset class="osint-mode"><legend>⚙ Настройки отображения результатов</legend><label><input type="radio" name="ai" value="0" checked><span><b>Все</b><small>Полный поиск</small></span></label><label><input type="radio" name="ai" value="0"><span><b>Без ИИ</b><small>Только сырые данные</small></span></label><label><input type="radio" name="ai" value="1"><span><b>С ИИ</b><small>Краткое резюме и анализ</small></span></label></fieldset><button class="submit" type="button">⌕ &nbsp; Запустить поиск <b>→</b></button></form><div id="osint-status" class="osint-status" role="status" aria-live="polite"></div><div id="osint-results" class="osint-results"></div><details class="osint-notice"><summary>Условия использования</summary><p>Результаты могут быть неполными и не подтверждают личность владельца username. Используйте инструменты только законно, с разрешением и с учётом правил сайтов.</p></details></section>' if section == 'user-search' else '')}
